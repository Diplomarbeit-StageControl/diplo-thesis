...
\fancyfoot[R]{% Autoren rechts
	\ifthenelse{\value{page} < 25}{Pirringer}{%
		\ifthenelse{\value{page} < 33}{Becksteiner}{%
			\ifthenelse{\value{page} < 48}{Pospichal}{%
				\ifthenelse{\value{page} < 71}{Becksteiner}{%
					\ifthenelse{\value{page} < 88}{Pospichal}{%
						\ifthenelse{\value{page} < 108}{Pirringer}{}
					}%
				}%
			}%
		}%
...
Der Anchor ist ein fixer Referenzpunkt, der für die Positionsbestimmung benötigt wird. Um diesen zu initialisieren, muss im Code folgende Zeile hinzugefügt werden.
...
Diese Skizze zeigt einen Auszug aus der Positionsbestimmung mit Hilfe von Trilateration und das Zusammenspiel zwischen Anchor und Tag. 
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.4\linewidth]{images/Titelseite.png}}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.5\linewidth]{images/Informationsseite.png}}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.4\linewidth]{images/Teamseite.png}}
...
Basierend auf diesen Prozentwerten wandelt der Raspberry Pi die Daten in Winkelgrade um und steuert die Servomotoren entsprechend. Dadurch wird die Tonausrichtung automatisch an die veränderte Position der Künstlerin oder des Künstlers angepasst. Dieses System ermöglicht eine präzise Klangsteuerung.
