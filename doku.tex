		...
		\ifthenelse{\value{page} < 34}{Becksteiner}{%
			\ifthenelse{\value{page} < 48}{Pospichal}{%
				\ifthenelse{\value{page} < 67}{Becksteiner}{%
					\ifthenelse{\value{page} < 83}{Pospichal}{%
						\ifthenelse{\value{page} < 102}{Pirringer}{}
					}%
				}%
			}%
		}%
...
Bei dem yAnchor gehen wir immer von einem \texttt{x = 0} aus, um die Grenze des Koordinatensystems zu setzen und somit negative Schnittpunkte direkt herauszufiltern. Und setzen den \texttt{y} Wert auf die letzte gemessene Distanz vom Base Anchor. Dieser Wert wird durch die Methode \texttt{getLastArrayElement(baseAnchorRangeArray)} ermittelt.
\begin{lstlisting}[style=Java, caption=Berechnung yAnchor, captionpos=b]
         yAnchor[1] = getLastArrayElement(baseAnchorRangeArray);
...
\newpage
Bei dem xAnchor funktioniert das Ganze etwas anders und ist deutlich komplizierter. Hierbei muss man sich zwei Schnittpunkte vorstellen und diese mithilfe der Kreisgleichung und einer quadratischen Gleichung berechnen. Mit den Methoden \texttt{getY1(), getX()} berechnen wir die genaue Koordinate bei diesem Punkt.

\begin{lstlisting}[style=Java, caption=getX und getY1 Berechnung, captionpos=b]
	private double getY1(double distanceYX, double distanceBaseX) {
		double yDistance = yAnchor[1];
		return Math.abs((yDistance * yDistance + distanceBaseX * distanceBaseX - distanceYX * distanceYX) /(2 * yDistance));
	}
	
	private double getX(double distanceBaseAnchorXAnchor, double y1) {
		return Math.abs(Math.sqrt((distanceBaseAnchorXAnchor*distanceBaseAnchorXAnchor) - (y1*y1)));
	}
\end{lstlisting}

\begin{lstlisting}[style=Java, caption=Berechnung xAnchor, captionpos=b]
	private void setXAnchorPosition() {
	        double distanceYAnchorXAnchor = getLastArrayElement(yAnchorRangeArray);
	double distanceBaseAnchorXAnchor = getLastArrayElement(baseAnchorRangeArray);
	...
	System.out.println("distanceYAnchorXAnchor: " + distanceYAnchorXAnchor + "distanceBaseAnchorXAnchor: " + distanceBaseAnchorXAnchor);
...
Jede Klasse bekommt einen Controller. Zum Beispiel: \texttt{Main.java} hat einen Controller \texttt{MainController.java}. Das hilft dabei einen guten Überblick über das Programm und seine Funktionalitäten zu behalten. Die Methoden werden in Java-Klassen umgesetzt und dann in einem FXML file aufgerufen, um zu funktionieren. Da Netzwerkoperationen durchgeführt werden, wurde die Klasse \texttt{NetworkSocketListener} erstellt, um den Empfang von Daten in einem separaten Thread des Programms auszuführen. Zudem wurde die Klasse \texttt{NetworkSocketSender} implementiert, die für das Senden von Daten an den Raspberry Pi verantwortlich ist.
