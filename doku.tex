...
Ein weiterer Apekt dieser Diplomarbeit ist die gleichzeitige Ansteuerung von Lichtanlagen, wie z.b einem Spotlight (\emph{deutsch Verfolgungsscheinwerfer}), das den Künstler auf der Bühne verfolgt. Auf Bühnen wird nicht nur ein Sporlight eingesetzt, sondern ganze Anlagen von Lichtern. Dies wird gemacht, um für den Zuschauer eine immersive Vorstellung zu bieten. 

\subsection{Lichtanlagen bei StageControl}
Im Fall von StageControl wird keine vollständige Lichtanlage verwendet; es kommt lediglich ein Spotlight zum Einsatz, um die Beleuchtung des Künstlers zu ermöglichen. So erhalten wird die perfekte Abstimmung des Tons und des Lichts auf den Künstler.

\subsection{Ansteuerung von Lichtanlagen}
Die gängiste Art der Ansteuerung von Lichtanlagen ist DMX-Protokoll (\emph{Digital Multiplex}) oder RDM-Protokoll (\emph{Remote Device Management}). Zur Ansteuerung der Lichtanlgen selbst, wird ein DMX-Controller benötigt, der die Scheinwerfer steuert. Es wird ein DMX Controller mit einem DMX-Ausgang und Schweinwerfer mit einem DMX-Eingang benötigt.\\
\textbf{DMX} ist das am häufisten Verwendete Protokoll zur Lichansteuerung in der Bühnentechnik. Moving-Heads, Lichteffekte und viele mehr werden in der Regel mit Hilfe von Lichtsteuerpulten über das DMX-Protokoll angesteuert und/oder programmiert. \\
\textbf{RDM} basiert auf dem DMX-PRotokoll erlaubt  aber im Halbduplex Rückmeldungen, so kann das Einrichten deutlich erleichtert werden.
